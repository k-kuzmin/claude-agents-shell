...
public interface IShellResolver
{
    ...
    /// <remarks>
    /// Порядок — тот же, в котором резолвер перебирает провайдеров при откате, поэтому
    /// первый элемент и есть та оболочка, в которой запустится сессия, если выбранная
    /// не найдена. На этом держится предупреждение в диалоге настроек проекта: реализация,
    /// которая отсортирует список «для красоты», сделает предупреждение враньём.
    /// </remarks>
