...
public sealed class TabStripViewModel : ObservableObject
{
    ...
    /// <para>
    /// «Первая» — в порядке открытия, а **не** в порядке попадания в «ждёт ввода»: из двух
    /// ждущих победит открытая раньше, даже если ждать она начала позже. Видимая полоса
    /// строится фильтрацией того же списка с сохранением порядка, поэтому внутри проекта это
    /// ровно самая левая ждущая вкладка. Порядок не зависит от того, как пользователь
