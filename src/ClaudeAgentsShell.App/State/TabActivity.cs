...
/// потока — ещё и конец его хода или новый промпт. Граница сессии снимает все диалоги:
/// сессия сменилась, и поздний <c>PostToolBatch</c> не должен поднять устаревшее состояние.
/// </para>
/// </remarks>
internal sealed class TabActivity
{
    ...
    /// <para>
    /// Снимается только диалог главного потока: новый ход главного потока означает, что его
    /// диалога на экране уже нет. Диалоги сабагентов остаются — человек при открытом диалоге
    /// промпт отправить не может, значит, этот UserPromptSubmit прислал сам Claude Code
    /// (/loop, расписание, машинное сообщение), а диалог сабагента всё ещё ждёт ответа.
    /// Пока он открыт, вкладка остаётся «ждёт ввода», а «работает» запоминается как место
    /// возврата.
    /// </para>
    /// </remarks>
    private TabState? OnPromptSubmitted()
    {
        ...
        return Settle(TabState.Busy, closed, publishWhenNoDialogs: true);
    }
