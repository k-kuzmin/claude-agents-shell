...
Проект пишется агентами Claude Code по [техническому заданию](docs/TZ.md). Главная сессия —
оркестратор: фиксирует контракты, раздаёт блоки сабагентам, прогоняет ревью и аудит
производительности (определения агентов — в `.claude/agents/`, правила — в
[`.claude/CLAUDE.md`](.claude/CLAUDE.md)). Состояние, принятые решения, отступления от ТЗ и
план приёмки — в [`docs/PROGRESS.md`](docs/PROGRESS.md).

Каждый этап — ветка `stage/…` и PR в `main`. Мерж в `main` (кроме изменений только в документации) собирает релиз: версия `0.2.<номер
запуска>`, самодостаточный архив для win-x64, а описание релиза берётся из раздела
«Заметки к релизу» описания PR.
